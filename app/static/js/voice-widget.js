    ...
    handleAction(action, data) {
        ...
        switch (action) {
            ...
            case 'cancel_complete':
                this.displayCancellation(data);
                break;
            // Add other actions here
        }
    }

    displayCancellation(data) {
        const html = `
            <div class="card mt-2 border-danger shadow-sm">
                <div class="card-header bg-danger text-white py-1 px-2 small">
                    <i class="fas fa-times-circle me-1"></i>Cancellation Confirmed
                </div>
                <div class="card-body p-2 small">
                    <p class="mb-1"><strong>PNR:</strong> ${data.pnr}</p>
                    <p class="mb-0"><strong>Status:</strong> <span class="badge bg-danger">${data.status}</span></p>
                    <p class="mt-2 mb-0 text-muted italic text-center" style="font-size: 0.8rem;">Refund has been initiated.</p>
                </div>
            </div>
        `;
